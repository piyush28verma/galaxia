"""
Local Test Suite for Galaxia
Sends test queries to the room to verify Galaxia answers properly.
"""
import os
import sys
import time
...
if __name__ == "__main__":
    print("Select a test case to send to @Galaxia:")
    ...
    if choice == "1":
        snippet = "@Galaxia review: def calculate_discount(price, discount): return price - (price * discount / 100) if discount < 100 else 0"
        send_test_message(snippet)
    elif choice == "2":
        snippet = "@Galaxia review: SELECT * FROM users WHERE username = '\" + user_input + \"' AND password = '\" + pwd + \"'"
        send_test_message(snippet)
    elif choice == "3":
        snippet = "@Galaxia review: def find_duplicates(nums): return [x for x in nums if nums.count(x) > 1]"
        send_test_message(snippet)
    elif choice == "4":
        send_test_message("@Galaxia how much do your audits cost?")
    else:
